...
# Leemos el directorio video aplicando el filtro
def input(cadena):
    flag = False
    if cadena.endswith('.mp4') and not cadena.startswith('NO'):
        flag = True
    return flag

# print(os.listdir('./video'))
entrada = list(filter(input, os.listdir('./video')))
print(entrada)
...
for element in entrada:
    ...
    *El tamaño del marco es importante porque deben ser las dimensiones de los marcos que tiene en la memoria que desea 
    ...
    out_mp4 = cv2.VideoWriter('out'+element, cv2.VideoWriter_fourcc(*'XVID'), 10, (frame_width, frame_height))
    ...
    def display_objects(im, objects, threshold=0.90):
        rows = im.shape[0]
        cols = im.shape[1]
        ...
        for i in range(objects.shape[2]):
            ...
            if score > threshold:
                ...
                cv2.rectangle(im, (x, y), (x + w, y + h), (255, 255, 255),
                              2)  # introduce en la imagen un rectángulo blanco
